...
class OpenRouterProvider(BaseProvider):
    ...
    async def chat_completion(self, request, timeout: float = 30.0) -> ChatResponse:
        if not self._api_key:
            raise PermanentProviderError("OpenRouter API key not configured")

        payload = request.provider_payload()
        payload["transforms"] = ["middle-out"]  # OpenRouter optimisation hint
    ...
    def _parse_response(self, resp: httpx.Response, model: str) -> ChatResponse:
        ...
        if resp.status_code in (500, 502, 503, 504):
            raise TemporaryProviderError(f"OpenRouter server error: {resp.status_code}")
        if resp.status_code >= 400:
            raise PermanentProviderError(f"OpenRouter error {resp.status_code}: {resp.text[:200]}")
